...
function Hitbox:SetOwner(owner: Model)
	self.Owner = owner
	local params = self.RaycastParams or RaycastParams.new()
	table.insert(params.FilterDescendantsInstances, { self.Owner })
end
		...
		for i = #ActiveHitboxes, 1, -1 do
			...
			for _: number, point: Point in ipairs(ActiveHitboxes[i].HitboxRaycastPoints) do
				...
				if raycastResult then
					...
					if model == ActiveHitboxes[i].Owner then
						continue
					end
